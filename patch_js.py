...
if '// =========== 全局状态 ===========' not in js:
    var_block = '''// =========== 全局状态 ===========
let _connectionsCache = [];   // 连接列表缓存（用于搜索 + 批量）
let _batchMode = false;        // 是否处于批量模式
let _selectedConns = new Set(); // 批量选中的连接 ID 集合

'''
    idx = js.find('function esc(')
    if idx >= 0:
        js = js[:idx] + var_block + js[idx:]
        print("JS: inserted global state variables")
    else:
        print("WARN: function esc() not found, cannot insert vars")

# ===== 2. 替换 loadConnections() + 插入新函数 =====
old_load = '''async function loadConnections() {
...
function renderConnections() {
    ...
    grid.innerHTML = filtered.map(c => {
        ...
        const checkboxHtml = _batchMode
            ? \`<input type="checkbox" class="batch-checkbox" ${isSelected ? "checked" : ""} onclick="toggleConnSelect(\'${c.id}\', this.checked)" />\`
            : \'\';
        return `
...
if old_load in js:
    js = js.replace(old_load, new_code, 1)
    print("JS: loadConnections + batch ops replaced")
else:
    print("JS: WARN - old loadConnections pattern not found, trying alternate...")
    # 可能是换行符问题，尝试用更灵活的方式
    if 'async function loadConnections()' in js:
        print("  But async function loadConnections() IS found - maybe whitespace issue")
    else:
        print("  async function loadConnections() NOT found at all!")

open(js_path, 'w', encoding='utf-8').write(js)
print("JS written to disk.")
